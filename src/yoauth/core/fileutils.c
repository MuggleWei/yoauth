										  ...
										  unsigned long *outlen)
{
	...
	if (hdr->ver != YOAUTH_DATA_VERSION) {
		LOG_ERROR("invalid yoauth data version");
		free(plaintext);
		plaintext = NULL;
		goto load_exit;
	}
	if (hdr->data_type != YOAUTH_DATA_TYPE_TOTP) {
		LOG_ERROR("invalid data type");
		free(plaintext);
		plaintext = NULL;
		goto load_exit;
	}
	if (hdr->data_size != sizeof(yoauth_totp_data_t)) {
		LOG_ERROR("incorrect data size");
		free(plaintext);
		plaintext = NULL;
		goto load_exit;
	}
	if (*outlen != sizeof(yoauth_head_t) + hdr->cnt * hdr->data_size) {
